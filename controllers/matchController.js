...
export async function getMatches(req, res) {
  try {
    ...
    const userIds = [...new Set((candidates || []).map(c => c.user_id))].filter(id => id !== uid);
    ...
    const { data: skills, error: err4 } = await supabase.from('skills').select('id,name');
    if (err4) throw err4;
    const skillMap = Object.fromEntries((skills || []).map(s => [s.id, s.name]));

    // 4) build result: aggregate matched skills per user
    const result = (profiles || []).map(p => {
      const matchedSkillIds = (candidates || []).filter(c => c.user_id === p.user_id).map(c => c.skill_id);
      const matchedSkillNames = matchedSkillIds.map(id => skillMap[id]).filter(Boolean);
      return {
        user_id: p.user_id,
...
export async function createMatch(req, res) {
  try {
    ...
    if (!user_a || !user_b) return res.status(400).json({ error: 'user_a & user_b required' });
